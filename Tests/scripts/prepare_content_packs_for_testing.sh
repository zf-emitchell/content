...
if [ ! -n "${NIGHTLY}" ]; then
  ...
  if [ ! -f $CONTENT_PACKS_TO_INSTALL_FILE ]; then
    ...
    if [[ -z "$CONTENT_PACKS_TO_INSTALL" ]]; then
      ...
      python3 ./Tests/Marketplace/upload_packs.py -a $PACK_ARTIFACTS -d $CIRCLE_ARTIFACTS/packs_dependencies.json -e $EXTRACT_FOLDER -b $GCS_BUILD_BUCKET -s $KF -n $CIRCLE_BUILD_NUM -p $CONTENT_PACKS_TO_INSTALL -o -sb $TARGET_PATH -k $PACK_SIGNING_KEY -rt false --id_set_path $ID_SET -rt true
