...
def main():
    ...
    username = options.user if options.user else secret_conf_file.get('username')
    password = options.password if options.password else secret_conf_file.get('userPassword')
    server = Server(host=hosts[0], user_name=username, password=password)
    error_msg = 'Failed to set marketplace configuration.'
    ...
    install_all_content_packs(client=server.client, host=str(server), prints_manager=ParallelPrintsManager(1))
